...
Imported rows are tagged `form_location='mailchimp_import'` so they're distinguishable from real form submissions. Missing fields (name, street, city, state) are inserted as empty strings. Re-runs are idempotent — rows with the same `(email, created_at)` are skipped.
